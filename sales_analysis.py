...
def create_pivot_analysis_with_comparison(data, date_range, view_type='Weekly'):
    ...
    # Calculate previous period dates based on view type
    if view_type == 'Monthly':
        # Calculate number of months in current period
        months_diff = (end_date.year - start_date.year) * 12 + end_date.month - start_date.month
        
        # For monthly view where data is on first of month
        previous_end = start_date - pd.DateOffset(days=1)
        previous_end = previous_end.replace(day=1)  # First day of the previous period's end month
        previous_start = previous_end - pd.DateOffset(months=months_diff)
